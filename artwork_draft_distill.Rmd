...
a single work in the museum. The list of percentiles below show that fewer than 50%
...
x %>% 
  select(CreditLine) %>% 
  mutate(CreditLine = tolower(CreditLine)) %>% 
  separate_rows(CreditLine, sep = " ") %>% 
  mutate(CreditLine = str_replace_all(CreditLine, "[^a-z]", "")) %>% 
  group_by(CreditLine) %>% 
  summarize(number = n()) %>% 
  arrange(desc(number)) %>%
  filter(!(CreditLine %in% c("of", "the", "and", "e", "", "c", "in", "on"))) 

x %>% 
  select(CreditLine) %>% 
  filter(str_detect(CreditLine, "[Cc]ollection"))
